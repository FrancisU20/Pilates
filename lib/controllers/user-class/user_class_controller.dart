...
class UserClassController {
  ...
  //? Funcion para actualizar el estado de una clase
  Future<StandardResponse<UserClassModel>> updateUserClass(
      String userClassId, UpdateStatusModel updateStatusModel) async {
    try {
      final apiBase = await ApiBaseService.create(contentType: 'json');

      String dataJson = jsonEncode(updateStatusModel.toJson());
      Logger.logSendData(dataJson);

      final response = await apiBase.patch('/users-classes/$userClassId',
      ...
      return updateUserClassResponse;
    } catch (e) {
      throw Exception(e.toString().replaceAll('Exception: ', ''));
    }
  }
